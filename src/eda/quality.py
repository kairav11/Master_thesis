...
def basic_report(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Simple numeric summaries for key columns if present
...
def _parse_times(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    # Try strict format first, then fallback
...
def _clip_outliers(out: pd.DataFrame) -> pd.DataFrame:
    # Remove invalid or extreme ATA and distance
    if "ata_minutes" in out.columns:
        s = pd.to_numeric(out["ata_minutes"], errors="coerce")
        # drop negative and zero durations
        out = out[s > 0].copy()
        # clip very extreme (e.g., > 99th percentile)
        if s.notna().sum() > 50:
            hi = s.quantile(0.99)
...
def _sanitize_coords(out: pd.DataFrame) -> pd.DataFrame:
    # Ensure lat/lng within plausible bounds if present
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop exact duplicates on key identifiers if present
