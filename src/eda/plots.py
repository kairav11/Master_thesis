...
def _apply_hatch_to_bars(ax: plt.Axes) -> None:
    """Apply alternating hatch patterns to bar-like patches for colorblind-friendly patterns."""
    hatches = ["/", "\\", "|", "-", "+", "x", "o", "O", ".", "*"]
...
def _apply_hatch_to_boxplot(ax: plt.Axes) -> None:
    """Apply hatches to box artists when available."""
    hatches = ["/", "\\", "|", "-", "+", "x", "o", "O", ".", "*"]
...
def generate_eda_plots(df: pd.DataFrame, out_dir: str, title: str) -> None:
    _ensure_dir(out_dir)
    # Remove obvious outliers on ATA via IQR and drop nulls for stability
    work = df.copy()
...
def _plot_order_hour_distribution(df: pd.DataFrame, out_dir: str, title: str) -> None:
    # Derive hour if needed from accept_time
    work = df.copy()
